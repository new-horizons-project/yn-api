...
from ..db       import get_session, schema, topic as topic_db
...
async def search_topics(
	...
	sort:   str = Query("title", regex="^(title|created_at)$"),
...
@router_poblic.get("/{topic_id}", response_model=topics.TopicBase)
...
@router_poblic.get("/{topic_id}/translations", response_model=list[topics.TopicTranslationBase])
async def get_topic(topic_id: int, db: AsyncSession = Depends(get_session)):
	return await topic_db.get_topic_translations_list(topic_id, db)

@router_poblic.get("/{topic_id}/translations/{translation_id}", response_model=topics.TopicTranslationBase)
...
@router_poblic.get("/topics/{topic_id}/tags", response_model = list[tag.TagBase])
